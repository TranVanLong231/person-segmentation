...
def get_arguments():
    ...
    RESTORE_FROM = './dataset/CE2P/resnet101-imagenet.pth'
    SAVE_NUM_IMAGES = 2
...
def train(loader, valid_loader, model, opt, scaler, criterion, total_iters, epoch, args):
    ...
    for idx, batch in enumerate(loop):         
        ...
        hgt = hgt.float().cuda()
        wgt = wgt.float().cuda()
        hwgt = hwgt.float().cuda()
        opt.zero_grad(set_to_none = True)

        with torch.cuda.amp.autocast_mode.autocast():

            preds = model(imgs)
            #print(f"Loss {idx} {[torch.unique(labels), torch.unique(edges)]} {[torch.unique(hgt), torch.unique(wgt), torch.unique(hwgt)]}")
...
def main():
    ...
    torch.save(model.state_dict(), args.snapshot_dir+'model_CHIP.pth')
