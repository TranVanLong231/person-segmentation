...
class LIPDataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        im_path = os.path.join(self.root, self.dataset + '_images','Images', im_name + '.jpg')
        
        parsing_anno_path = os.path.join(self.root, self.dataset + '_segmentations','Category_ids', im_name + '.png')
        ...
        if self.dataset != 'test': 
            ...
            parsing_anno[parsing_anno==22] =0
    ...
    def getItemFromBuffer(self,index):
        if(len(self.buffer100)<=index):            
            print('buffer out, load more...')
            self.bufferOffset += len(self.buffer100)
            self.buffer100 = []
            i = self.bufferOffset            
            while(i<self.__len__() and len(self.buffer100) <100):
                data = self.getItemOne(i)
                self.buffer100.append(data)
                i += 1                            
            print(f"Load complete, buff length {len(self.buffer100)}, buff offset {self.bufferOffset}")
        return self.buffer100[index-self.bufferOffset]
